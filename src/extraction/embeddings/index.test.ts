...
describe('Embeddings Module', () => {
  ...
  describe('DEFAULT_ACTIVITY_QUERIES', async () => {
    ...
    it('contains activity-related queries', () => {
      ...
      expect(DEFAULT_ACTIVITY_QUERIES.some((q) => q.includes('باید برویم'))).toBe(true)
      expect(DEFAULT_ACTIVITY_QUERIES.some((q) => q.includes('chúng ta nên'))).toBe(true)
      expect(DEFAULT_ACTIVITY_QUERIES.some((q) => q.includes('כדאי שנלך'))).toBe(true)
      expect(DEFAULT_ACTIVITY_QUERIES.some((q) => q.includes('เราควรไป'))).toBe(true)
    })
  })
})
